...
export class MiscMagicRepository {
  ...
  getRandom(count: number = 1): IFooModel[] {
    ...
    for (let index = 0; index < count; index++) {
      const table = [
        ...
        this.miscMagic2Table,
        this.miscMagic3Table,
        this.miscMagic4Table,
        this.miscMagic5Table,
      ].getRandom();
  ...
  getMisc3(count: number = 1): IFooModel[] {
    if (count < 1) count = 1;
    const processor = new MiscMagic3Processor(this.dice, this.miscMagic3Table);
    const items: IFooModel[] = processor.getMiscMagic(count);
    return items;
  }
...
class MiscMagic1Processor {
  ...
  private getProcessed(row: IRowResult2<ITableRow>): IFooModel[] {
    ...
    switch (true) {
      ...
      case row.Index === 18:
        results.push(...this.getBootsOfDancing('1d100r<37r>55'));
        break;
      default:
...
class MiscMagic2Processor {
  constructor(
    private dice: IDice,
    private misc2table: MyTable<ITableRow>
  ) {}
...
class MiscMagic3Processor {
  constructor(
    private dice: IDice,
    private misc3table: MyTable<ITableRow>
  ) {}
  ...
  private getInsturmentOfTheBards(): string {
    return 'Instrument of the Bards';
  }
}
...
class MiscMagic4Processor {
  constructor(
    private dice: IDice,
    private misc4table: MyTable<ITableRow>
  ) {}
...
class MiscMagic5Processor {
  constructor(
    private dice: IDice,
    private misc5table: MyTable<ITableRow>
  ) {}
  ...
  getMiscMagic(count: number): IFooModel[] {
    ...
    for (let index = 0; index < count; index++) {
      const roll = this.dice.roll(this.misc5table.DieExpression);
      const row = this.misc5table.find(roll.total);
      const obj = this.getProcessed(row);
      items.push(...obj);
    }
